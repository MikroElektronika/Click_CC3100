...
#elif defined( TI )
sbit CC3100_CS_PIN  at GPIO_PORTB_AHB_DATA.B4;
sbit CC3100_RST_PIN at GPIO_PORTD_AHB_DATA.B2;
sbit CC3100_HIB_PIN  at GPIO_PORTF_AHB_DATA.B5;

sbit CC3100_CS_PIN_Direction  at GPIO_PORTB_AHB_DIR.B4;
sbit CC3100_RST_PIN_Direction at GPIO_PORTD_AHB_DIR.B2;
sbit CC3100_HIB_PIN_Direction  at GPIO_PORTF_AHB_DIR.B5;
#endif
...
void system_init()
{
    ...
    #elif defined( TI )
    GPIO_Digital_Output( &GPIO_PORTB_AHB, _GPIO_PINMASK_4 ); // CC3100 CS
    GPIO_Digital_Output( &GPIO_PORTD_AHB, _GPIO_PINMASK_2 ); // CC3100 RST Dummy
    GPIO_Digital_Output( &GPIO_PORTF_AHB, _GPIO_PINMASK_5 ); // CC3100 HIB
    GPIO_Digital_Input( &GPIO_PORTB_AHB, _GPIO_PINMASK_2 );  // CC3100 INT
    ...
    SPI1_Init_Advanced( 15000000, _SPI_MASTER, _SPI_8_BIT | _SPI_CLK_IDLE_LOW |
                       _SPI_SECOND_CLK_EDGE_TRANSITION,
                       &_GPIO_MODULE_SPI1_B5E45_AHB );
                       
    GPIO_PORTB_AHB_AFSEL |= ( 1 << _GPIO_PINCODE_2 );  // Enable alternate pin functions
    GPIO_PORTB_AHB_IM    |= ( 1 << _GPIO_PINCODE_2 );  // Set mask to pin 2
    GPIO_PORTB_AHB_IEV   |= ( 1 << _GPIO_PINCODE_2 );  // Rising Edge will trigger interrupt
    ...
    UART1_Init_Advanced( 115200, 10000, _UART_8_BIT_DATA, _UART_NOPARITY,
                        _UART_ONE_STOPBIT, _UART_HIGH_SPEED, 
                        &_GPIO_MODULE_UART1_Q45 );
...
#elif defined( TI )
void irq_handler() iv IVT_INT_GPIOB ics ICS_AUTO
{
    if( GPIO_PORTB_AHB_RIS & ( 1 << _GPIO_PINCODE_2 ))
    {
        GPIO_PORTB_AHB_ICR |= ( 1 << _GPIO_PINCODE_2 );
        ( *isr_event )( 0 );
    }
}
